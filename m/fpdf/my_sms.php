...
//Second table: specify 4 columns
		$sql5 = mysql_query("SELECT COUNT(*) from order_sms_master where s_ord_date = '$date'");
		...
		$pdf->SetXY(30, 33);
		$pdf->AddCol('s_ord_no',55,'Order No','L');
		$pdf->AddCol('s_ord_by',40,'Order by','L');	
		$pdf->AddCol('s_ent_by',40,'Entry By','L');
		...
		$pdf->Table("SELECT m.s_ord_date, m.s_ord_no, m.s_ord_by, m.s_ent_by, m.s_tre_id, t.t_name
					FROM order_sms_master AS m, territory_info AS t
					WHERE m.s_tre_id = t.t_id
					AND m.s_ord_date = '$date' order by t.t_name",$prop);
