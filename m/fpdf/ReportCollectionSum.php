		...
		while ($row=mysql_fetch_array($result)) {
				...
				if($e_id=='all'){
					...
					$result1 = mysql_query("SELECT z.z_id, z.z_name, z.z_bn_name, SUM(p.pay_amount) AS TotalBill, SUM(p.bill_discount) AS TotalDiscount FROM payment AS p 
								...
								WHERE c.z_id = '$ZoneId' AND p.pay_date BETWEEN '$f_date' AND '$t_date' AND c. mac_user = '0'
								GROUP BY c.z_id"); 
					...
					$result2 = mysql_query("SELECT z.z_id, z.z_name, SUM(b.amount) AS SignupAmount FROM bill_signup AS b 
								...
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE c.z_id = '$ZoneId' AND b.pay_date BETWEEN '$f_date' AND '$t_date' AND c. mac_user = '0'
								GROUP BY c.z_id"); 
					...
					$result1 = mysql_query("SELECT z.z_id, z.z_name, z.z_bn_name, SUM(p.pay_amount) AS TotalBill, SUM(p.bill_discount) AS TotalDiscount FROM payment AS p 
								LEFT JOIN clients AS c ON c.c_id = p.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE c.z_id = '$ZoneId' AND p.pay_date BETWEEN '$f_date' AND '$t_date' AND p.pay_ent_by = '$e_id' AND c. mac_user = '0'
								GROUP BY c.z_id"); 
					$row1 = mysql_fetch_array($result1);
					...
					$result2 = mysql_query("SELECT z.z_id, z.z_name, SUM(b.amount) AS SignupAmount FROM bill_signup AS b 
								LEFT JOIN clients AS c ON c.c_id = b.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE c.z_id = '$ZoneId' AND b.pay_date BETWEEN '$f_date' AND '$t_date' AND b.ent_by = '$e_id' AND c. mac_user = '0'
								GROUP BY c.z_id"); 
					$row2 = mysql_fetch_array($result2);
				}
