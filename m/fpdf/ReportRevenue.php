...
if($z_id == 'all'){
	class PDF extends PDF_MySQL_Table
		...
		$pdf->AddPage();
			...
			$pdf->Cell(205,6,'Revenue Form '.$FastDate.' To '.$LastDate,'LTBR',0,'C',true);
			$pdf->Ln();
			$pdf->SetX(2);
			$pdf->Cell(15,6,'S/L','LTB',0,'C',true);
			$pdf->Cell(70,6,'Zone Name','LTB',0,'L',true);
			$pdf->Cell(40,6,'Actual Bill','LTB',0,'R',true);
			$pdf->Cell(40,6,'Signup Charge','LTB',0,'R',true);
			$pdf->Cell(40,6,'Total','LTBR',0,'R',true);
			$pdf->Ln();
			
		$result = mysql_query("SELECT l.z_name, l.z_bn_name, l.TotalBill, l.TotalDiscount, t.SignupAmount FROM
								(
								...
								LEFT JOIN
								(
								SELECT z.z_id, z.z_name, SUM(b.amount) AS SignupAmount FROM bill_signup AS b 
								LEFT JOIN clients AS c ON c.c_id = b.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE b.pay_date BETWEEN '$f_date' AND '$t_date' AND c. mac_user = '0'
								GROUP BY c.z_id
								)t
								ON l.z_id = t.z_id");  							
		...
		$pdf->AddPage();
			...
			$pdf->SetX(2);
			$pdf->Cell(15,6,'S/L','LTB',0,'C',true);
			$pdf->Cell(70,6,'Client ID','LTB',0,'L',true);
			$pdf->Cell(40,6,'Actual Bill','LTB',0,'R',true);
			$pdf->Cell(40,6,'Signup Charge','LTB',0,'R',true);
			$pdf->Cell(40,6,'Total','LTBR',0,'R',true);
			$pdf->Ln();
			
		$result = mysql_query("SELECT l.c_id, l.z_name, l.z_bn_name, l.TotalBill, l.TotalDiscount, t.SignupAmount FROM
								(
								SELECT c.c_id, c.c_name, z.z_id, z.z_name, z.z_bn_name, SUM(b.bill_amount) AS TotalBill, SUM(b.discount) AS TotalDiscount FROM billing AS b 
								LEFT JOIN clients AS c ON c.c_id = b.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								...
								LEFT JOIN
								(
								SELECT c.c_id, z.z_id, z.z_name, SUM(b.amount) AS SignupAmount FROM bill_signup AS b 
								LEFT JOIN clients AS c ON c.c_id = b.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE b.pay_date BETWEEN '$f_date' AND '$t_date' AND c.z_id = '$z_id' AND c. mac_user = '0'
								GROUP BY c.c_id
								)t
								ON l.c_id = t.c_id");  							
