...
if($z_id == 'all'){
	class PDF extends PDF_MySQL_Table
		...
		function Header()
		{
			...
			$this->Cell(15,6,'Payable','LTBR',0,'C',true);
		...
		$result = mysql_query("SELECT b.id, b.c_id, c.address, c.cell, c.note, SUM(b.p_price) AS p_price, SUM(c.discount) AS discount, SUM(b.adv_bill) AS adv_bill, SUM(b.bill_amount) AS amt, SUM(b.pay_amount) AS pay, SUM(b.bill_discount) AS dsc 
								FROM billing AS b
								LEFT JOIN clients AS c ON c.c_id = b.c_id
								WHERE b.sts = 0 AND b.bill_sts = 0 AND b.bill_date <= '$f_date' AND b.p_id != '13'
		...
		while ($row=mysql_fetch_array($result)) {
				...
				$pdf->Cell(20,5,number_format($p_price,0),'LTB',0,'R',true); 
				$pdf->Cell(15,5,number_format($due,0),'LTB',0,'R',true);
				$pdf->Cell(15,5,number_format($adv_bill,0),'LTB',0,'R',true);
				$pdf->Cell(15,5,number_format($discount_all,0),'LTB',0,'R',true);
				$pdf->Cell(15,5,number_format($tot_pay,0),'LTB',0,'R',true);
				$pdf->Cell(15,5,number_format($payable,0),'LTBR',0,'R',true);
			...
			$pdf->Cell(20,6,number_format($act_bill,0),'LTB',0,'R',true); 
			$pdf->Cell(15,6,number_format($due_bill,0),'LTB',0,'R',true);
			$pdf->Cell(15,6,number_format($advanc_bill,0),'LTB',0,'R',true);
		...
		$result = mysql_query("SELECT b.id, b.c_id, c.address, c.cell, c.note, SUM(b.p_price) AS p_price, SUM(c.discount) AS discount, SUM(b.adv_bill) AS adv_bill, SUM(b.bill_amount) AS amt, SUM(b.pay_amount) AS pay, SUM(b.bill_discount) AS dsc 
								FROM billing AS b
								LEFT JOIN clients AS c ON c.c_id = b.c_id
								WHERE b.sts = 0 AND b.bill_sts = 0 AND c.z_id = '$z_id' AND b.bill_date <= '$f_date' AND b.p_id != '13'
								GROUP BY c_id ORDER BY c.address");
