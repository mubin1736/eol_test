...
if($z_id == 'all'){
	class PDF extends PDF_MySQL_Table
		...
		$pdf->AddPage();
			...
			$pdf->Cell(205,6,'Advance Form '.$FastDate.' To '.$LastDate,'LTBR',0,'C',true);
			$pdf->Ln();
			$pdf->SetX(2);
			$pdf->Cell(15,6,'S/L','LTB',0,'C',true);
			$pdf->Cell(150,6,'Zone Name','LTB',0,'L',true);
			$pdf->Cell(40,6,'Total Advance','LTBR',0,'R',true);
			$pdf->Ln();
			
		$result = mysql_query("SELECT l.z_name, l.TotalBill, t.TotalBills FROM
								(
								...
								WHERE b.bill_date BETWEEN '$f_date' AND '$t_date'
								GROUP BY c.z_id
								)l
								LEFT JOIN
								(
								SELECT z.z_id, z.z_name, SUM(p.pay_amount) AS TotalBills, SUM(p.bill_discount) AS TotalDiscount FROM payment AS p 
		...
		while ($row=mysql_fetch_array($result)) {
				...
				$TotalBill = $TotalBill1 - $TotalBill2;
		...
		function Footer()
				{
					...
					$this->SetFont('Helvetica','I',8);
					//Page number
					$this->Cell(0,10,'Page '.$this->PageNo().'/ExpartOnline','T',0,'C');
					$this->SetDrawColor(8, 102, 198);
					parent::Footer();
				}
		}
		...
		$pdf->AddPage();
			...
			$pdf->Cell(205,6,'Dew Form '.$FastDate.' To '.$LastDate,'LTBR',0,'C',true);
			$pdf->Ln();
			$pdf->SetX(2);
			$pdf->Cell(15,6,'S/L','LTB',0,'C',true);
			$pdf->Cell(150,6,'Zone Name','LTB',0,'L',true);
			$pdf->Cell(40,6,'Total Dew','LTBR',0,'R',true);
			$pdf->Ln();
		...
		$result = mysql_query("SELECT l.z_name, l.TotalBill, t.TotalBills FROM
								...
								LEFT JOIN clients AS c ON c.c_id = b.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE b.bill_date BETWEEN '$f_date' AND '$t_date' AND c.z_id = '$z_id'
								GROUP BY c.z_id
								)l
								LEFT JOIN
								(
								...
								LEFT JOIN clients AS c ON c.c_id = p.c_id 
								LEFT JOIN zone AS z ON z.z_id = c.z_id
								WHERE p.pay_date BETWEEN '$f_date' AND '$t_date' AND c.z_id = '$z_id'
								GROUP BY c.z_id
								)t
								ON l.z_id = t.z_id");  							
